...
; IfMsgBox, NO, Return
 ...
 if !IsObject(file)
 {
  ...
  {
    this_id := id%A_Index%
	WinGet, min_max_setting, MinMax, ahk_id %this_id%
	win_minimized := IfEqual min_max_setting, -1
	...
	{
		line=Title="%this_title%"`,x=%x%`,y=%y%`,width=%width%`,height=%height%`,maximized=%min_max_setting%,path=""`r`n
		file.Write(line)
   	}
