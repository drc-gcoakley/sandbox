...
def zipPath = ["c:/iat/la_input_files/Math Content/Math/"
               ,"c:/iat/la_input_files/ELA Content/ELA/"
               ,"c:/iat/la_input_files/Science Content/Science/"
                ]
...
File duplicateFile = new File(imageMapFilePath)
duplicateFile.write JsonOutput.prettyPrint(json)
...
def int createImagesUnzipped(File zipFile) {
    ...
    zf.entries().findAll { !it.directory }.each {
        ...
        if (it.toString().startsWith("images")) {
            ...
            Globals.itemNameMap.put(it.name,newname)
        }
    }
    return 0
}
