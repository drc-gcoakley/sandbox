	...
	*/
	...
	static function isRewritableRequest(oSession: Session) {
		var rewritable = (oSession.HTTPMethodIs("GET") || oSession.HTTPMethodIs("POST")) && 
			...
			! oSession.oResponse.headers.ExistsAndContains('Content-Type', 'application/') && 
			! oSession.uriContains('.woff');
		//FiddlerObject.log('httpToS: rewritable request: ' + oSession.fullUrl);
		return rewritable;
	}
	...
	static function isRewritableURL(oSession: Session) {
		for (var i = 0; i < httpToSSites.length; ++i) {
			...
			if (oSession.uriContains(site)) {
				//FiddlerObject.log('httpToS: matching URL: ' + oSession.fullUrl);
				return true;
			}
		}
		return false;
		...
		if (isRewritableRequest(oSession) && isRewritableURL(oSession)) {
			...
			for (var i = 0; i < httpToSSites.length; ++i) {
				...
				if (newResponse.length != response.length) {
					...
					FiddlerObject.log('httpToS: Changing "http:" to "https:" of ' + 
						site + ' found in ' + oSession.fullUrl);
				} else {
					FiddlerObject.log('httpToS: Skipping. ' + regex + 
			...
			FiddlerObject.log('httpToS: request does NOT match: ' + oSession.fullUrl);
		}
		if (responseChanged) {
