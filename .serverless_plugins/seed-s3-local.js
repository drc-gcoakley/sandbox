  ...
  seedHandler() {
    ...
    if (this.config && this.config.sources) {
      this.config.sources.forEach((config) => {
        ...
        this.serverless.cli.log(`S3LocalSeederPlugin: Loading PNG files from directory '${sourceDirectory}' into S3 bucket '${bucketName}'.`);
        ...
        fs.readdirSync(sourceDirectory).forEach(async (file) => {
          ...
          if (allowedFileTypes && !allowedFileTypes.includes(fileExt)) {
            console.log(`Skipping seeding of unspecified file type of ${fileExt} which is not included in ${fileTypes}`);
          } else {
        ...
        if (fileCount === 0) {
          this.serverless.cli.log(`S3LocalSeederPlugin: No PNG files processed!`);
        } else {
          this.serverless.cli.log(`S3LocalSeederPlugin: Done reading ${fileCount} PNG file(s)'.`);
        }
      });
    }
